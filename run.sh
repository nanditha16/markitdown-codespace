...
echo "      Save response → prompts/ats_recommend_prompt_response.txt"
echo ""
echo "   ── EVIDENCE PASS (Stage 3.5) ─────────────────────────────────────"
echo "   Drop evidence files into input/evidence/ first:"
echo "     Career_Wealth.xlsx, iRecon_pointers.pdf, project notes, etc."
echo ""
echo "   ./scripts/ingest_evidence.sh"
echo "      → converts evidence to output/career_wealth_chunk/*.md"
echo "      (run once per corpus update, not every ATS cycle)"
echo ""
echo "   ./scripts/ats_evidence_gap.sh \"output/JD.md\" \"<variant_name>\""
echo "      → prompts/ats_evidence_gap_prompt.txt  [UPLOAD TO CLAUDE.AI]"
echo "      Surfaces real experience never captured in any resume variant"
echo ""
